...
function example() {
    let numInput = documnet.getElementById('grossInput').value;
    numInput=numInput.toLowerCase();
    let y = document.getElementsByClassName('labelFlex');
    let ychild = document.getElementsByClassName('areaFlex');
    for(i = 0; i < y.length; i++) {
        if (!y[i].innerHTML.toLowerCase().includes(numInput)) {
...
//     while (AnnualIncome == 'number') {
//             let GrossMonthly = AnnualIncome / 12;
//             let HousingAffordability = GrossMonthly * 0.33;
//             document.getElementById('GrossMonthly').innerHTML.innerText = GrossMonthly;
//     }
// }
